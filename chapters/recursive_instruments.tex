...
\section{A complete recursive case pattern}

Imagine a ten-card Cicero reading whose eighth position, the surrounding world's
response, is difficult to read. The reader does not clarify the whole spread
again. They make that position the parent object and ask English playing cards
for worldly detail.

The English three-card line shows a force passing, a force present, and a force
approaching. The middle card is a Seven of Diamonds: a small material
interruption, child, minor dispute, or detail demanding care. Because the
question concerns a stored program, the reader asks the Christmas Tree to
inspect that Seven as a local operation. The Tree's crown supplies ignition; its
root supplies the stopping condition. A Marseille Minor then gives the Seven's
material geometry: perhaps a crossing, enclosure, or asymmetrical exchange.

The result returns through three languages:

\begin{enumerate}
\item Cicero says where in the larger path the problem belongs;
\item English says what worldly operation is occurring;
\item the Tree and Marseille say how the operation is organized and embodied.
\end{enumerate}

If the three layers agree, the reader has not proved fate. They have found a
stable description and a practical next question. If they disagree, the
disagreement is the result. The reader records it, asks whether another
instrument can see the missing condition, or reaches $T\_STOP$ with the door
open.

That is the recursive garden at its best: not a machine for accumulating
answers, but a way of bringing different kinds of attention to the same living
place until the place can be entered.
